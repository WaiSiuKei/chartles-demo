...
import { aT as LongPositionToolType, ap as IIntlService } from "./index-NZHt9VGv.js";
...
class PositionLongPrimitive extends PositionBasePrimitive {
  stopPrice() {
  ...
  _amountTarget(accountSize, targetPrice, entryPrice, quantity, pointValue) {
    ...
    return roundValue(accountSize + (targetPrice - entryPrice) * quantity * pointValue / rate);
  }
  _amountStop(accountSize, stopPrice, entryPrice, quantity, pointValue) {
    ...
    return roundValue(accountSize - (entryPrice - stopPrice) * quantity * pointValue / rate);
  }
}
class LongPositionTool extends DrawingTool {
  constructor() {
    super(...arguments);
    __publicField(this, "type", LongPositionToolType);
  }
  createPrimitive(session) {
    const level = calculateLevel(session);
    return new PositionLongPrimitive(
      {
        id: this.id,
        points: [],
        linecolor: "#808080",
